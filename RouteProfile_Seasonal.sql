...
FROM (
    ...
    FROM (
        ...
        FROM (
            SELECT
                CASE
                    WHEN dd.FullDate >= CAST('2025-12-01' AS DATETIME)
                     AND dd.FullDate < CAST('2026-03-01' AS DATETIME)
                        THEN '01-Winter: Dec-Feb'

                    WHEN dd.FullDate >= CAST('2026-03-01' AS DATETIME)
                     AND dd.FullDate < CAST('2026-06-01' AS DATETIME)
                        THEN '02-Spring: Mar-May'

                    WHEN dd.FullDate >= CAST('2026-06-01' AS DATETIME)
                     AND dd.FullDate < CAST('2026-09-01' AS DATETIME)
                        THEN '03-Summer: Jun-Aug'

                    WHEN dd.FullDate >= CAST('2026-09-01' AS DATETIME)
                     AND dd.FullDate < CAST('2026-12-01' AS DATETIME)
                        THEN '04-Fall: Sep-Nov'
            ...
            WHERE
                dd.FullDate >= CAST('2025-12-01' AS DATETIME)
                AND dd.FullDate < CAST('2026-12-01' AS DATETIME)
...
ON apc."Season Period" = fare."Season Period"
